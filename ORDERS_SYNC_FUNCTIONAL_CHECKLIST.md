# Orders Sync Functional Checklist

Scheduler key: `orders-sync`  
Primary service: `src/api/services/OrdersSchedulerService.ts`  
Admin route base: `/scheduler/orders`  
...
- Delta Exchange active orders and history reference: `https://docs.delta.exchange/`

Use this checklist to verify the full behavior of the `orders-sync` scheduler, its user-scoped scheduler record, and the trust split between the admin scheduler surface and the user-owned `/orders` product surface.

## 1. Identity And Ownership

- [ ] Scheduler record exists with key `orders-sync`.
- [ ] Scheduler name is `Orders Sync`.
- [ ] Scheduler description clearly states that it is the system reconciliation scheduler for broker order snapshots, checkpoints, and repair replay tooling.
- [ ] Scheduler is normalized to `schedulerType = user`.
- [ ] Attempts to switch it to `global` scope are rejected.
- [ ] Shared scheduler or cron execution uses the user-scoped scheduler record and the all-users path for all eligible users' connections.
- [ ] Scheduler or cron execution does not collapse to the admin actor's personal broker scope.
- [ ] Any legacy or partially migrated scheduler rows are normalized back to the expected user-scoped shape for this scheduler mode.
- [ ] If scoped replay is supported, it remains an operational subset of the shared scheduler, not a user-owned product-page run.

## 2. Route And Access

- [ ] Admin-only route works through `/scheduler/orders/config`.
- [ ] Admin-only route works through `/scheduler/orders/run`.
- [ ] Admin-only route works through `/scheduler/orders/pause`.
- [ ] Admin-only route works through `/scheduler/orders/resume`.
- [ ] Admin-only route works through `/scheduler/orders/stop`.
- [ ] Admin-only route works through `/scheduler/orders/restart`.
- [ ] Admin-only route works through `/scheduler/orders/runs`.
- [ ] Admin-only route works through `/scheduler/orders/sync-state`.
- [ ] Admin-only route works through `/scheduler/orders/sync-state/summary`.
- [ ] Admin-only route works through `/scheduler/orders/runs/:runId/progress`.
- [ ] Admin-only route works through `/scheduler/orders/runs/:runId/updates`.
- [ ] Admin-only route works through `/scheduler/orders/runs/:runId/updates/export`.
- [ ] Product route works through `/orders/futures/refresh`.
- [ ] Product route works through `/orders/futures/sync-status`.
- [ ] Product route works through `/orders/overview`.
- [ ] Internal execution route works through `/internal/orders/sync`.
...
- [ ] `lookbackDays` can be updated within supported bounds.
...
- [ ] Invalid scope change to `global` is rejected with a clear validation error.
- [ ] Invalid schedule combinations fail with a clear validation error.
...
- [ ] Scheduler or cron runs fetch orders across all eligible active broker accounts for all users, not only the admin actor who clicked run.
...
- [ ] `/orders/futures/refresh` only refreshes the signed-in user's own active broker accounts.
- [ ] `/orders/futures/refresh` respects optional `brokerKey` and `accountId` only within the signed-in user's own scope.
- [ ] `/orders/futures/refresh` returns `NotFound` when the requested account does not belong to the signed-in user.
- [ ] `/orders/futures/refresh` returns an idle or no-op response when the signed-in user has no connected or idle routes.
- [ ] `/orders/futures/sync-status` reports only the signed-in user's own accounts.
- [ ] `/orders/overview` remains hydrated from the signed-in user's own scope.
- [ ] Product-page refresh delegates to `/internal/orders/sync` with `targetUserIds: [userId]`, not all users.

## 7. Scope Separation And Trust Boundary

- [ ] Scheduler or cron scope never leaks all-users data into `/orders` responses for a signed-in user.
...
- [ ] Support and ops teams can tell from logs whether an order update came from the admin scheduler path or the user desk path.

## 8. Manual Run And Replay Behavior
...
- [ ] Default manual scheduler run still executes against the all-users account set, not the admin actor's own connections, even though the scheduler record is user-scoped.
- [ ] Scoped repair or replay, when used, is clearly logged as scoped operational work and not confused with a product-page user refresh.
- [ ] `resetCheckpoint` behavior is explicit and logged when a scoped replay is requested.
...
- [ ] `GET /runs` returns paginated run logs for only `orders-sync`.
...
- [ ] Current item block identifies the current account, owner, broker, or order-scope being processed when available.
...
- [ ] Recent run section behaves as expected, showing the latest run state, timing, counters, replay flags, and error details without stale or mismatched data.
...
- [ ] Update rows preserve enough identity to trace the affected account, owner user, broker, or order where applicable.
...
## 12. Sync-State, Checkpoints, And Diagnostics

- [ ] `GET /sync-state` returns paginated orders sync-state rows for `orders-sync`.
- [ ] Sync-state can be filtered by `accountId`.
- [ ] Sync-state can be filtered by `ownerUserId`.
- [ ] Sync-state can be filtered by `brokerKey`.
- [ ] If legacy `userId` query alias is still supported, it maps cleanly without ambiguity.
- [ ] `GET /sync-state/summary` returns a truthful aggregate summary for the scheduler.
- [ ] Orders policy exposes lookback and overlap rules consistently.
- [ ] Checkpoint state is updated only for the correct account.
- [ ] Scoped replay with checkpoint reset is reflected in run metadata, logs, and operational messaging.
- [ ] Replay tooling does not erase unrelated accounts' checkpoint state.
...
- [ ] Replay or checkpoint-reset failures emit scheduler alerts.
- [ ] Alert throttling prevents duplicate high-frequency failure alerts.
- [ ] Alert source is `orders-sync`.
...
- [ ] A successful scheduler run fetches orders across all eligible users' connected broker accounts.
- [ ] A successful product refresh fetches orders only for the requesting user's own eligible broker accounts.
- [ ] Scheduler runs do not cross-write one owner's orders into another owner's snapshot or sync state.
- [ ] Open, pending, partially filled, filled, cancelled, rejected, expired, and failed statuses are normalized consistently.
- [ ] Checkpoint updates stay tied to the correct `accountId`, `brokerKey`, and owner user.
- [ ] Open-order and history-order fetches are combined without duplicating or losing order identities.
- [ ] Order deduplication preserves the highest-fidelity latest state.
...
- [ ] A broken broker route for one account does not silently erase orders for unrelated owners.
- [ ] Failure path writes activity and alert records.

## 19. Recommended Proof Run

- [ ] Fetch config and verify the shared scheduler shape intended for all eligible users' accounts.
...
- [ ] Verify `/orders/futures/refresh` for a signed-in test user only touches that user's own accounts.
- [ ] Verify a signed-in user cannot use `/orders` refresh to trigger an all-users run.
- [ ] Queue a scoped replay with checkpoint reset and verify the run metadata and operational messaging reflect the replay.

## 20. Fast Regression Set

- [ ] Existing orders scheduler phase tests pass.
...
- [ ] Replay and checkpoint-reset behavior return expected shapes.
- [ ] `/orders/futures/refresh` remains user-owned and does not widen to all-users scope.
