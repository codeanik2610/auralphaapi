...
import { selectDeltaProtectionRepairApplyCandidates } from '../maintenance/repair-suggested-trades-delta-protection';

type JsonRecord = Record<string, unknown>;

const STALE_CANCEL_ACTION = 'would_cancel_stale_protection_orders';
const OUTPUT_FILE = String(
  process.env.SUGGESTED_TRADES_DELTA_STALE_PROTECTION_WATCHDOG_OUTPUT_FILE ||
    'artifacts/suggested-trades-delta-stale-protection-watchdog.json'
).trim();
const LIMIT = Math.max(
  1,
  Math.floor(
    Number(
      process.env.SUGGESTED_TRADES_DELTA_STALE_PROTECTION_WATCHDOG_LIMIT ||
        process.env.SUGGESTED_TRADES_DELTA_PROTECTION_REPAIR_LIMIT ||
        25
    )
  )
);
const MAX_STALE_CANCEL_CANDIDATES = Math.max(
  0,
  Math.floor(Number(process.env.SUGGESTED_TRADES_MAX_DELTA_STALE_CANCEL_CANDIDATES || 0))
);
...
function summarizeItem(item: DeltaProtectionRepairPreviewItem): JsonRecord {
  return {
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const staleCancelCandidates = selectDeltaProtectionRepairApplyCandidates(
      previewReport.items,
      LIMIT,
      { includeStaleCancel: true }
    ).filter((item) => item.remediation.action === STALE_CANCEL_ACTION);
    const applyEnabled = readBooleanFlag('SUGGESTED_TRADES_DELTA_PROTECTION_REPAIR_APPLY');
    ...
    console.log('suggested-trades-delta-stale-protection-watchdog:', JSON.stringify(report));
    ...
    if (thresholdExceeded) {
      console.error(
        `Delta stale protection watchdog found ${staleCancelCandidateItems} stale cancel candidate(s).`
      );
      process.exit(1);
    }
  } finally {
