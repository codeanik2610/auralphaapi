#!/usr/bin/env bash
set -euo pipefail

CONTAINER_NAME="${AURALPHA_API_CONTAINER_NAME:-auralpha-auralpha-api-1}"
ARTIFACT_DIR="${AURALPHA_MUDREX_STALE_PROTECTION_WATCHDOG_ARTIFACT_DIR:-/opt/auralpha/guardrail-artifacts/mudrex-stale-protection-watchdog}"
RETENTION_DAYS="${AURALPHA_MUDREX_STALE_PROTECTION_WATCHDOG_RETENTION_DAYS:-14}"
LOOKBACK_DAYS="${SUGGESTED_TRADES_MUDREX_PROTECTION_LOOKBACK_DAYS:-7}"
LIMIT="${SUGGESTED_TRADES_MUDREX_STALE_PROTECTION_WATCHDOG_LIMIT:-25}"
MAX_STALE_CANCEL_CANDIDATES="${SUGGESTED_TRADES_MAX_MUDREX_STALE_CANCEL_CANDIDATES:-0}"

timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "${ARTIFACT_DIR}"

tmp_output="${ARTIFACT_DIR}/${timestamp}.tmp"
log_output="${ARTIFACT_DIR}/${timestamp}.log"
json_output="${ARTIFACT_DIR}/${timestamp}.json"
container_json_output="/tmp/auralpha-mudrex-stale-protection-${timestamp}.json"

cleanup_tmp() {
  rm -f "${tmp_output}"
}
trap cleanup_tmp EXIT

set +e
docker exec \
  -e SUGGESTED_TRADES_MUDREX_PROTECTION_REPAIR_APPLY=false \
  -e SUGGESTED_TRADES_MUDREX_STALE_PROTECTION_CANCEL_APPLY=false \
  -e SUGGESTED_TRADES_MUDREX_PROTECTION_LOOKBACK_DAYS="${LOOKBACK_DAYS}" \
  -e SUGGESTED_TRADES_MUDREX_STALE_PROTECTION_WATCHDOG_LIMIT="${LIMIT}" \
  -e SUGGESTED_TRADES_MAX_MUDREX_STALE_CANCEL_CANDIDATES="${MAX_STALE_CANCEL_CANDIDATES}" \
  -e SUGGESTED_TRADES_MUDREX_STALE_PROTECTION_WATCHDOG_OUTPUT_FILE="${container_json_output}" \
  "${CONTAINER_NAME}" \
  node dist/scripts/checks/check-suggested-trades-mudrex-stale-protection-watchdog.js \
  >"${tmp_output}" 2>&1
status=$?
set -e

mv "${tmp_output}" "${log_output}"
trap - EXIT

if docker cp "${CONTAINER_NAME}:${container_json_output}" "${json_output}" >/dev/null 2>&1; then
  docker exec "${CONTAINER_NAME}" rm -f "${container_json_output}" >/dev/null 2>&1 || true
else
  docker exec "${CONTAINER_NAME}" rm -f "${container_json_output}" >/dev/null 2>&1 || true
  json_line="$(sed -n 's/^suggested-trades-mudrex-stale-protection-watchdog: //p' "${log_output}" | tail -n 1)"
  if [[ -n "${json_line}" ]]; then
    printf '%s\n' "${json_line}" >"${json_output}"
  fi
fi

find "${ARTIFACT_DIR}" -type f -mtime +"${RETENTION_DAYS}" -delete

if [[ "${status}" -ne 0 ]]; then
  echo "Mudrex stale protection watchdog failed; see ${log_output}" >&2
  exit "${status}"
fi

if [[ ! -s "${json_output}" ]]; then
  echo "Mudrex stale protection watchdog produced no JSON artifact; see ${log_output}" >&2
  exit 1
fi

echo "Mudrex stale protection watchdog passed; artifact=${json_output}"
