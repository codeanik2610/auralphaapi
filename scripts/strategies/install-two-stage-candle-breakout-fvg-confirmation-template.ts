...
import {
  buildTwoStageCandleBreakoutFvgConfirmationTemplatePayload,
  TWO_STAGE_CANDLE_BREAKOUT_FVG_CONFIRMATION_TEMPLATE_LEGACY_NAMES,
  TWO_STAGE_CANDLE_BREAKOUT_FVG_CONFIRMATION_TEMPLATE_NAME,
} from '../../src/api/strategies/templates/TwoStageCandleBreakoutFvgConfirmationTemplate';
...
const installTemplate = async (): Promise<InstallResult> => {
  ...
  const payload = buildTwoStageCandleBreakoutFvgConfirmationTemplatePayload();
  ...
  return strategyDataSource.transaction(async (manager) => {
    ...
    const existingRows = await templateRepository
      ...
      .setParameter('currentName', TWO_STAGE_CANDLE_BREAKOUT_FVG_CONFIRMATION_TEMPLATE_NAME)
      .getMany();
