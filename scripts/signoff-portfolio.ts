...
const GATE_FILE = String(
  process.env.PORTFOLIO_SIGNOFF_GATE_FILE || 'artifacts/portfolio-release-gate.json'
).trim();
const OUTPUT_FILE = String(
  process.env.PORTFOLIO_SIGNOFF_OUTPUT_FILE || 'artifacts/portfolio-signoff.json'
).trim();
const STEP_SUMMARY_FILE = String(process.env.GITHUB_STEP_SUMMARY || '').trim();
const REQUIRE_LIVE_HEALTH =
  String(process.env.PORTFOLIO_SIGNOFF_REQUIRE_LIVE_HEALTH || 'false')
    .trim()
    .toLowerCase() === 'true';
const MANUAL_REVIEW_WORKFLOW_VERIFIED =
  String(process.env.PORTFOLIO_SIGNOFF_MANUAL_REVIEW_WORKFLOW_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const REPORT_EXPORT_VERIFIED =
  String(process.env.PORTFOLIO_SIGNOFF_REPORT_EXPORT_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const SHAREABLE_WORKSPACE_VERIFIED =
  String(process.env.PORTFOLIO_SIGNOFF_SHAREABLE_WORKSPACE_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const RECONCILIATION_RUNBOOK_VERIFIED =
  String(process.env.PORTFOLIO_SIGNOFF_RECONCILIATION_RUNBOOK_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const APPROVER = String(process.env.PORTFOLIO_SIGNOFF_APPROVER || '').trim();
const STAGING_WORKFLOW_URL = String(
  process.env.PORTFOLIO_SIGNOFF_STAGING_WORKFLOW_URL || ''
).trim();
const DASHBOARD_URL = String(process.env.PORTFOLIO_SIGNOFF_DASHBOARD_URL || '').trim();
const RUNBOOK_URL = String(process.env.PORTFOLIO_SIGNOFF_RUNBOOK_URL || '').trim();
const RELEASE_NOTE_URL = String(
  process.env.PORTFOLIO_SIGNOFF_RELEASE_NOTE_URL || ''
).trim();
...
async function writeStepSummary(summary: {
  ...
  const lines = [
    ...
    `- required portfolio suites passed: ${summary.checks.requiredSuitesPassed ? 'yes' : 'no'}`,
    `- live portfolio health reviewed: ${summary.checks.liveHealthReviewed ? 'yes' : 'no'}`,
    `- manual review workflow verified: ${summary.checks.manualReviewWorkflowVerified ? 'yes' : 'no'}`,
    `- report export verified: ${summary.checks.reportExportVerified ? 'yes' : 'no'}`,
    `- shareable workspace verified: ${summary.checks.shareableWorkspaceVerified ? 'yes' : 'no'}`,
    `- reconciliation runbook verified: ${summary.checks.reconciliationRunbookVerified ? 'yes' : 'no'}`,
    '',
    '### Evidence',
    '',
...
async function run(): Promise<void> {
  ...
  const liveHealthReviewed = REQUIRE_LIVE_HEALTH
    ? resultsByKey.get('backend-portfolio-live-health')?.status === 'passed'
    : true;
  ...
  const checks = {
    ...
    liveHealthReviewed,
    manualReviewWorkflowVerified: MANUAL_REVIEW_WORKFLOW_VERIFIED,
    reportExportVerified: REPORT_EXPORT_VERIFIED,
    shareableWorkspaceVerified: SHAREABLE_WORKSPACE_VERIFIED,
    reconciliationRunbookVerified: RECONCILIATION_RUNBOOK_VERIFIED,
  };
  ...
  assert.equal(
    ...
    true,
    'required portfolio suites, lint, and build checks must all pass'
  );
  assert.equal(
    checks.liveHealthReviewed,
    true,
    'live portfolio health must be reviewed when PORTFOLIO_SIGNOFF_REQUIRE_LIVE_HEALTH is true'
  );
  assert.equal(
    checks.manualReviewWorkflowVerified,
    true,
    'manual rebalance review workflow must be explicitly verified before final portfolio sign-off'
  );
  assert.equal(
    checks.reportExportVerified,
    true,
    'workspace report generation and export must be explicitly verified before final portfolio sign-off'
  );
  assert.equal(
    checks.shareableWorkspaceVerified,
    true,
    'shareable portfolio workspace state must be explicitly verified before final portfolio sign-off'
  );
  assert.equal(
    checks.reconciliationRunbookVerified,
    true,
    'the portfolio reconciliation runbook must be explicitly verified before final portfolio sign-off'
  );
  ...
  console.log('portfolio-signoff:', JSON.stringify(summary));
}
