...
const FRONTEND_CWD = String(
  process.env.OVERVIEW_FRONTEND_CWD || '/Users/apple/Documents/Project/Frontend/aurAlphaApp'
).trim();
const OUTPUT_FILE = String(
  process.env.OVERVIEW_RELEASE_GATE_OUTPUT_FILE || 'artifacts/overview-release-gate.json'
).trim();
const RUN_LIVE_CHECKS =
  String(process.env.OVERVIEW_RUN_LIVE_CHECKS || '').trim().toLowerCase() === 'true';
...
async function run(): Promise<void> {
  ...
  const checks: GateCheckDefinition[] = [
    {
      key: 'backend-overview-contract',
      label: 'Backend overview contract suite',
      cwd: BACKEND_CWD,
      command: [npmBin(), 'run', 'test:overview-contract'],
    },
    {
      key: 'backend-overview-resilience',
      label: 'Backend overview resilience suite',
      cwd: BACKEND_CWD,
      command: [npmBin(), 'run', 'test:overview-resilience'],
    },
    {
      key: 'backend-overview-phase4',
      label: 'Backend overview phase 4 suite',
      cwd: BACKEND_CWD,
      command: [npmBin(), 'run', 'test:overview-phase4'],
    },
    {
    ...
    {
      key: 'frontend-overview-eslint',
      label: 'Frontend overview lint check',
      cwd: FRONTEND_CWD,
      command: [
        npxBin(),
        'eslint',
        'src/pages/Dashboard/index.jsx',
        'src/pages/Dashboard/index.test.jsx',
        'src/store/slices/overviewSlice.js',
        'src/store/slices/overviewSlice.test.js',
        'tests/e2e/overview.spec.js',
      ],
    },
    {
      key: 'frontend-overview-ui',
      label: 'Frontend overview UI suite',
      cwd: FRONTEND_CWD,
      command: [
        npmBin(),
        'run',
    ...
    {
      key: 'frontend-overview-e2e',
      label: 'Frontend overview operator journey E2E',
      cwd: FRONTEND_CWD,
      command: [npmBin(), 'run', 'test:e2e', '--', 'tests/e2e/overview.spec.js'],
    },
    {
      key: 'backend-overview-live-health',
      label: 'Backend overview live health check',
      cwd: BACKEND_CWD,
      command: [npmBin(), 'run', 'check:overview-health'],
      enabled: RUN_LIVE_CHECKS,
      skipReason: 'OVERVIEW_RUN_LIVE_CHECKS is not true',
    },
  ];
  ...
  console.log('overview-release-gate:', JSON.stringify(summary));
