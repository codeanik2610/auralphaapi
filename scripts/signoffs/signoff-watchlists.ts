...
const GATE_FILE = String(
  process.env.WATCHLISTS_SIGNOFF_GATE_FILE || 'artifacts/watchlists-release-gate.json'
).trim();
const OUTPUT_FILE = String(process.env.WATCHLISTS_SIGNOFF_OUTPUT_FILE || '').trim();
const STEP_SUMMARY_FILE = String(process.env.GITHUB_STEP_SUMMARY || '').trim();
const MIN_SOAK_MINUTES = Math.max(
  0,
  Number(process.env.WATCHLISTS_SIGNOFF_MIN_SOAK_MINUTES || 0)
);
const DASHBOARDS_VERIFIED =
  String(process.env.WATCHLISTS_SIGNOFF_DASHBOARDS_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const OPERATOR_FLOW_VERIFIED =
  String(process.env.WATCHLISTS_SIGNOFF_OPERATOR_FLOW_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const OVERVIEW_VERIFIED =
  String(process.env.WATCHLISTS_SIGNOFF_OVERVIEW_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const APPROVER = String(process.env.WATCHLISTS_SIGNOFF_APPROVER || '').trim();
...
async function run(): Promise<void> {
  ...
  assert.equal(checks.testSuiteRan, true, 'watchlists release gate must run the dedicated suite');
  assert.equal(
    checks.samplesCollected,
    true,
    'watchlists release gate must collect at least one sample'
  );
  assert.equal(
    checks.minSoakSatisfied,
    true,
    `watchlists release gate soak must be at least ${MIN_SOAK_MINUTES} minutes`
  );
  assert.equal(checks.thresholdsSatisfied, true, 'watchlists release gate thresholds must pass');
  assert.equal(checks.dashboardsVerified, true, 'watchlists dashboards must be verified');
  assert.equal(checks.operatorFlowVerified, true, 'watchlists operator flow must be verified');
  assert.equal(checks.overviewVerified, true, 'watchlists overview must be verified');
  ...
  console.log('watchlists-signoff:', JSON.stringify(summary));
}
