...
const GATE_FILE = String(
  process.env.CONNECTIONS_SIGNOFF_GATE_FILE || 'artifacts/connections-release-gate.json'
).trim();
const OUTPUT_FILE = String(process.env.CONNECTIONS_SIGNOFF_OUTPUT_FILE || '').trim();
const STEP_SUMMARY_FILE = String(process.env.GITHUB_STEP_SUMMARY || '').trim();
const MIN_SOAK_MINUTES = Math.max(
  0,
  Number(process.env.CONNECTIONS_SIGNOFF_MIN_SOAK_MINUTES || 0)
);
const DASHBOARDS_VERIFIED =
  String(process.env.CONNECTIONS_SIGNOFF_DASHBOARDS_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const OPERATOR_FLOW_VERIFIED =
  String(process.env.CONNECTIONS_SIGNOFF_OPERATOR_FLOW_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const CREDENTIAL_FLOW_VERIFIED =
  String(process.env.CONNECTIONS_SIGNOFF_CREDENTIAL_FLOW_VERIFIED || '')
    .trim()
    .toLowerCase() === 'true';
const APPROVER = String(process.env.CONNECTIONS_SIGNOFF_APPROVER || '').trim();
...
async function run(): Promise<void> {
  ...
  const checks = {
    ...
    credentialFlowVerified: CREDENTIAL_FLOW_VERIFIED,
  };

  assert.equal(checks.testSuiteRan, true, 'connections release gate must run the dedicated suite');
  assert.equal(
    checks.samplesCollected,
    true,
    'connections release gate must collect at least one sample'
  );
  assert.equal(
    checks.minSoakSatisfied,
    true,
    `connections release gate soak must be at least ${MIN_SOAK_MINUTES} minutes`
  );
  assert.equal(checks.thresholdsSatisfied, true, 'connections release gate thresholds must pass');
  assert.equal(checks.dashboardsVerified, true, 'connections dashboards must be verified');
  assert.equal(checks.operatorFlowVerified, true, 'connections operator flow must be verified');
  assert.equal(
    checks.credentialFlowVerified,
    true,
    'connections credential flow must be verified'
  );
  ...
  await writeSummary([
    ...
    `- Credential flow verified: ${checks.credentialFlowVerified ? 'yes' : 'no'}`,
    '',
  ]);

  console.log('connections-signoff:', JSON.stringify(summary));
}
