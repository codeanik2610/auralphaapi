...
const GATE_FILE = String(
  process.env.SIGNALS_SIGNOFF_GATE_FILE || 'artifacts/signals-release-gate.json'
).trim();
const OUTPUT_FILE = String(process.env.SIGNALS_SIGNOFF_OUTPUT_FILE || '').trim();
const STEP_SUMMARY_FILE = String(process.env.GITHUB_STEP_SUMMARY || '').trim();
const MIN_SOAK_MINUTES = Math.max(0, Number(process.env.SIGNALS_SIGNOFF_MIN_SOAK_MINUTES || 0));
const DASHBOARDS_VERIFIED =
  String(process.env.SIGNALS_SIGNOFF_DASHBOARDS_VERIFIED || '').trim().toLowerCase() === 'true';
const OPERATOR_FLOW_VERIFIED =
  String(process.env.SIGNALS_SIGNOFF_OPERATOR_FLOW_VERIFIED || '').trim().toLowerCase() === 'true';
const SCAN_CONTROLS_VERIFIED =
  String(process.env.SIGNALS_SIGNOFF_SCAN_CONTROLS_VERIFIED || '').trim().toLowerCase() === 'true';
const APPROVER = String(process.env.SIGNALS_SIGNOFF_APPROVER || '').trim();
...
async function run(): Promise<void> {
  ...
  const checks = {
    ...
    scanControlsVerified: SCAN_CONTROLS_VERIFIED,
  };

  assert.equal(checks.testSuiteRan, true, 'signals release gate must run the dedicated suite');
  assert.equal(checks.samplesCollected, true, 'signals release gate must collect at least one sample');
  assert.equal(
    checks.minSoakSatisfied,
    true,
    `signals release gate soak must be at least ${MIN_SOAK_MINUTES} minutes`
  );
  assert.equal(checks.thresholdsSatisfied, true, 'signals release gate thresholds must pass');
  assert.equal(checks.dashboardsVerified, true, 'signals dashboards must be verified');
  assert.equal(checks.operatorFlowVerified, true, 'signals operator flow must be verified');
  assert.equal(checks.scanControlsVerified, true, 'signals scan controls must be verified');
  ...
  await writeSummary([
    ...
    `- Scan controls verified: ${checks.scanControlsVerified ? 'yes' : 'no'}`,
    '',
  ]);

  console.log('signals-signoff:', JSON.stringify(summary));
}
