...
const GATE_FILE = String(
  process.env.ACTIVITY_SIGNOFF_GATE_FILE || 'artifacts/activity-release-gate.json'
).trim();
const OUTPUT_FILE = String(process.env.ACTIVITY_SIGNOFF_OUTPUT_FILE || '').trim();
const STEP_SUMMARY_FILE = String(process.env.GITHUB_STEP_SUMMARY || '').trim();
const MIN_SOAK_MINUTES = Math.max(0, Number(process.env.ACTIVITY_SIGNOFF_MIN_SOAK_MINUTES || 0));
const DASHBOARDS_VERIFIED =
  String(process.env.ACTIVITY_SIGNOFF_DASHBOARDS_VERIFIED || '').trim().toLowerCase() === 'true';
const OPERATOR_FLOW_VERIFIED =
  String(process.env.ACTIVITY_SIGNOFF_OPERATOR_FLOW_VERIFIED || '').trim().toLowerCase() === 'true';
const EXPORTS_VERIFIED =
  String(process.env.ACTIVITY_SIGNOFF_EXPORTS_VERIFIED || '').trim().toLowerCase() === 'true';
const APPROVER = String(process.env.ACTIVITY_SIGNOFF_APPROVER || '').trim();
...
async function run(): Promise<void> {
  ...
  assert.equal(checks.testSuiteRan, true, 'activity release gate must run the dedicated suite');
  assert.equal(checks.samplesCollected, true, 'activity release gate must collect at least one sample');
  assert.equal(
    checks.minSoakSatisfied,
    true,
    `activity release gate soak must be at least ${MIN_SOAK_MINUTES} minutes`
  );
  assert.equal(checks.thresholdsSatisfied, true, 'activity release gate thresholds must pass');
  assert.equal(checks.dashboardsVerified, true, 'activity dashboards must be verified');
  assert.equal(checks.operatorFlowVerified, true, 'activity operator flow must be verified');
  assert.equal(checks.exportsVerified, true, 'activity exports must be verified');
  ...
  await writeSummary([
    ...
    `- Exports verified: ${checks.exportsVerified ? 'yes' : 'no'}`,
    '',
  ]);

  console.log('activity-signoff:', JSON.stringify(summary));
}
