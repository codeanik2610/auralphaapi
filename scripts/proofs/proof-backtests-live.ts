...
async function run(): Promise<void> {
  await runStep('lifecycle smoke', 'scripts/smokes/smoke-backtests-lifecycle.ts', {
    SMOKE_REQUIRE_BACKTEST_CHART:
      process.env.SMOKE_REQUIRE_BACKTEST_CHART || 'true',
  });

  await runStep('health threshold check', 'scripts/checks/check-backtests-health.ts');
}
