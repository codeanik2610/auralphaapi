...
@Entity({ name: 'risk_broker_snapshots' })
@Index('idx_risk_broker_snapshots_snapshot_id', ['snapshotId'])
@Index('uidx_risk_broker_snapshots_snapshot_broker', ['snapshotId', 'brokerKey'], {
  unique: true,
})
@Index('idx_risk_broker_snapshots_snapshot_risk_state_score', ['snapshotId', 'riskState', 'riskScore'])
export class RiskBrokerSnapshot {
  ...
  @Column({ name: 'tracked_balance', type: 'double', default: 0 })
  trackedBalance!: number;

  @Column({ name: 'wallet_balance', type: 'double', default: 0 })
  walletBalance!: number;

  @Column({ name: 'futures_balance', type: 'double', default: 0 })
