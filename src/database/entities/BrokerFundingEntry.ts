...
@Entity({ name: 'broker_funding_entries' })
@Unique('uq_broker_funding_entries_user_broker_account_external', [
  'userId',
  'brokerKey',
  'accountId',
  'externalId',
])
@Index('idx_broker_funding_entries_user_broker_account_occurred_at', [
  'userId',
  'brokerKey',
  'accountId',
  'occurredAt',
])
@Index('idx_broker_funding_entries_position_id', ['userId', 'brokerKey', 'accountId', 'positionId'])
@Index('idx_broker_funding_entries_suggested_trade', ['userId', 'suggestedTradeId'])
export class BrokerFundingEntry {
  ...
  @Column({ name: 'funding_rate_pct', type: 'decimal', precision: 18, scale: 8, nullable: true })
