...
@Entity({ name: 'risk_asset_snapshots' })
@Index('idx_risk_asset_snapshots_snapshot_id', ['snapshotId'])
@Index('uidx_risk_asset_snapshots_snapshot_symbol', ['snapshotId', 'symbol'], {
  unique: true,
})
@Index('idx_risk_asset_snapshots_snapshot_risk_state_score', ['snapshotId', 'riskState', 'riskScore'])
export class RiskAssetSnapshot {
  ...
  @Column({ name: 'broker_count', type: 'int', unsigned: true, default: 0 })
  brokerCount!: number;
