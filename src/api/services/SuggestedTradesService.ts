...
interface SuggestedTradeAutoExecutionBlockResult {
...
export class SuggestedTradesService {
  ...
  async blockAutoExecutionForAutomation(
    userId: string,
    suggestedTradeId: string,
    options: {
      executionMode: 'paper' | 'live';
  ...
  ): Promise<SuggestedTradeAutoExecutionBlockResult> {
    const validatedTradeId = validateSuggestedTradeId(suggestedTradeId);
    const trade = await this.suggestedTradeRepository.getSuggestedTradeById(
      userId,
      validatedTradeId
    ...
    return this.blockAutoExecutionForLoadedTrade(userId, trade, options);
  }

  private async blockAutoExecutionForLoadedTrade(
    userId: string,
    trade: SuggestedTrade,
    options: {
      executionMode: 'paper' | 'live';
    ...
    const message = String(options.reason || '').trim() || 'Auto execution blocked';
    const checkedAt = new Date().toISOString();
    await this.persistExecutionState(trade, {
      executionMode: options.executionMode,
      executionState: 'rejected',
      preTradeState: 'blocked',
      preTradeCheckedAt: checkedAt,
  ...
  private async maybeApplyLiveTrailingStop(
    ...
    if (brokerKey === 'delta_exchange') {
      if (!positionsAdapter.updateRiskOrder) {
        return this.recordTrailingStopError(
          trade,
          execution,
          nowIso,
          'Trailing SL update needs Delta reduce-only stop-loss replacement support.'
        );
      }
      if (!riskOrderIds.stopLossOrderId || !riskOrderIds.takeProfitOrderId) {
        const contextReason = riskOrderIds.unavailableReason
          ? ` ${riskOrderIds.unavailableReason}`
          : '';
        return this.recordTrailingStopError(
          trade,
          execution,
          nowIso,
          `Trailing SL update needs active Delta SL/TP order snapshots before replacement.${contextReason}`
        );
      }
    }
  ...
  private resolveDeltaProtectionOrderDetailFromReadback(
    ...
    const record =
      this.readRecordValue(rawRecord.data) ?? this.readRecordValue(rawRecord.result) ?? rawRecord;
    const quantity =
      this.readNumberValue(record.quantity) ??
      this.readNumberValue(record.size) ??
      this.readNumberValue(record.contracts);
    ...
    return {
      ...
      stopOrderType:
        this.readStringValue(record.stop_order_type) ?? this.readStringValue(record.stopOrderType),
    };
  }
      ...
      protectionPlan: {
        ...
        trailingStop: {
          ...
          lastVerification: {
            ...
            stopLossPrice:
              this.formatNumericString(verification.stopLossPrice) ?? verification.stopLossPrice,
            takeProfitPrice:
  ...
  private async resolveTrailingRiskOrderIdsForLiveUpdate(
    ...
    if (!orderId) {
      return {
        ...
        unavailableReason: 'Delta entry order id is missing.',
      };
    }
    ...
    const mismatchReason = this.resolveDeltaProtectionContextMismatchReason(protection, execution, {
      payload: positionPayload,
    });
    if (!this.hasUsableProtectionContext(protection) || mismatchReason) {
      return {
        stopLossOrderId: null,
        takeProfitOrderId: null,
        triggerType: fallback.triggerType,
        unavailableReason:
          mismatchReason ??
          `Delta linked protection is not active or complete (${describeLiveProtectionOrderContext(
            protection
          )}).`,
      };
    }
  ...
  private async maybeAutoCancelSiblingProtectionOrders(
    ...
    // Only cancel protection already linked to this execution. A symbol-wide live sweep can race
    // with a newer same-symbol Delta position before the position snapshot is visible.
    const activeProtectionOrderIds = Array.from(new Set(protection.activeOrderIds));
