...
const SCHEDULER_KEY = 'system-health-sync';
const HEALTH_SCHEDULER_OWNERSHIP = 'global' as const;
const HEALTH_SCHEDULER_DESCRIPTION =
...
@Service()
export class HealthCheckSchedulerService {
  ...
  async updateSchedulerConfig(
    ...
    try {
      ...
      if (
        ...
        String(payload.schedulerType || '').trim().toLowerCase() !== HEALTH_SCHEDULER_OWNERSHIP
      ) {
        throw new BadRequestAppError(
          'System Health Sync is a global system scheduler and cannot be switched to user scope.'
        );
      }
      ...
      const updated = await this.schedulerConfigRepository.updateByKey(SCHEDULER_KEY, {
        ...
        schedulerType: HEALTH_SCHEDULER_OWNERSHIP,
        config: nextConfig,
      });
      ...
      await this.logSchedulerActivity(
        actorUserId,
        'Health scheduler config updated',
        'Success',
      ...
      await this.logSchedulerActivity(
        actorUserId,
        'Health scheduler config update failed',
        'Failed',
  ...
  async runNow(actorUserId: string): Promise<ApiSuccessResponse<SchedulerRunNowResponse>> {
    try {
      ...
      const config = await this.ensureSchedulerConfig(timeZone);
      if (!config.enabled) {
        throw new BadRequestAppError('Health scheduler is paused. Resume it before running now.');
      }
      ...
      if (existingRunCommand) {
        ...
        return successResponse({
          ...
          message: 'Health scheduler run already queued',
        });
      }
      ...
      if (running) {
        return successResponse({
          ...
          executionMode: 'queue',
          started: false,
          message: 'Health scheduler run already in progress',
        });
      }
      ...
      return successResponse({
        ...
        runId,
        jobId: command.id,
        message: 'Health scheduler command queued',
      });
    } catch (error) {
      await this.logSchedulerActivity(
        actorUserId,
        'Health scheduler run queue failed',
        'Failed',
        error instanceof Error ? error.message : String(error)
      );
  ...
  async pauseScheduler(actorUserId: string): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    ...
    await this.logSchedulerActivity(actorUserId, 'Health scheduler paused', 'Success', `Paused ${SCHEDULER_KEY}`);
    return successResponse({
      queued: false,
      action: 'pause',
      message: 'Health scheduler paused',
      state: 'applied',
      commandIds: [],
    });
  }
  ...
  async resumeScheduler(actorUserId: string): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    ...
    await this.logSchedulerActivity(actorUserId, 'Health scheduler resumed', 'Success', `Resumed ${SCHEDULER_KEY}`);
    return successResponse({
      queued: false,
      action: 'resume',
      message: 'Health scheduler resumed',
      state: 'applied',
      commandIds: [],
    });
  }
  ...
  async stopScheduler(actorUserId: string): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    ...
    await this.logSchedulerActivity(
      actorUserId,
      'Health scheduler stop requested',
      'Success',
    ...
    return successResponse({
      ...
      message:
        running || cancelledPendingRuns > 0
          ? 'Health scheduler stop requested'
          : 'No active or queued health scheduler run to stop',
  ...
  async restartScheduler(actorUserId: string): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    ...
    const config = await this.ensureSchedulerConfig(timeZone);
    if (!config.enabled) {
      throw new BadRequestAppError('Health scheduler is paused. Resume it before restarting.');
    }
    if (env.scheduler.executionMode !== 'queue') {
    ...
    await this.logSchedulerActivity(
      actorUserId,
      'Health scheduler restart queued',
      'Success',
      running
    ...
    return successResponse({
      ...
      action: 'restart',
      state: 'queued',
      message: 'Health scheduler restart queued',
  ...
  private async ensureSchedulerConfig(_timeZone?: string): Promise<SchedulerConfig> {
    ...
    const config = await this.schedulerConfigRepository.createIfMissing({
      ...
      description: HEALTH_SCHEDULER_DESCRIPTION,
      enabled: false,
      cronExpression: '0 1 * * *',
    ...
    const requiresNormalization =
      normalizedSchedulerType !== HEALTH_SCHEDULER_OWNERSHIP ||
      String(config.description || '').trim() !== HEALTH_SCHEDULER_DESCRIPTION ||
