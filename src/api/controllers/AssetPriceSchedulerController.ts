...
import { AssetPriceSchedulerService } from '../services/AssetPriceSchedulerService';

@JsonController('/scheduler/asset-price')
@Service()
export class AssetPriceSchedulerController {
  @Inject(() => AssetPriceSchedulerService)
  private assetPriceSchedulerService!: AssetPriceSchedulerService;

  @Get('/config')
  async getConfig(@Req() request: Request): Promise<ApiSuccessResponse<SchedulerConfigResponse>> {
    return this.assetPriceSchedulerService.getSchedulerConfig(requireAdminAuthUser(request).userId);
  }
  ...
  async updateConfig(
    ...
    return this.assetPriceSchedulerService.updateSchedulerConfig(requireAdminAuthUser(request).userId, body);
  }

  @Post('/run')
  async runNow(
    @Req() request: Request,
    @Body() body?: Partial<UpdateSchedulerConfigBody>
  ): Promise<ApiSuccessResponse<SchedulerRunNowResponse>> {
    return this.assetPriceSchedulerService.runNow(requireAdminAuthUser(request).userId, body);
  }

  @Post('/pause')
  async pause(@Req() request: Request): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    return this.assetPriceSchedulerService.pauseScheduler(requireAdminAuthUser(request).userId);
  }

  @Post('/resume')
  async resume(@Req() request: Request): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    return this.assetPriceSchedulerService.resumeScheduler(requireAdminAuthUser(request).userId);
  }

  @Post('/stop')
  async stop(@Req() request: Request): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    return this.assetPriceSchedulerService.stopScheduler(requireAdminAuthUser(request).userId);
  }

  @Post('/restart')
  async restart(@Req() request: Request): Promise<ApiSuccessResponse<SchedulerControlResponse>> {
    return this.assetPriceSchedulerService.restartScheduler(requireAdminAuthUser(request).userId);
  }

  @Post('/purge-logs')
  async purgeLogs(@Req() request: Request): Promise<ApiSuccessResponse<SchedulerPurgeLogsResponse>> {
    return this.assetPriceSchedulerService.purgeSchedulerLogs(requireAdminAuthUser(request).userId);
  }
  ...
  async purgeLogsPreview(
    ...
    return this.assetPriceSchedulerService.getSchedulerPurgePreview(requireAdminAuthUser(request).userId);
  }
  ...
  async listRuns(
    ...
    return this.assetPriceSchedulerService.listSchedulerRuns(requireAdminAuthUser(request).userId, {
      limit,
      offset,
    });
  }
  ...
  async listAssets(
    ...
    return this.assetPriceSchedulerService.listSchedulerAssets({ limit, offset, search, assetId });
  }
  ...
  async listRunUpdates(
    ...
    return this.assetPriceSchedulerService.listSchedulerRunUpdates(
  ...
  async getRunProgress(
    ...
    return this.assetPriceSchedulerService.getSchedulerRunProgress(
      requireAdminAuthUser(request).userId,
      runId
    );
  }
  ...
  async exportRunUpdates(
    ...
    return this.assetPriceSchedulerService.exportSchedulerRunUpdates(
      requireAdminAuthUser(request).userId,
      runId,
      {
        actionType,
